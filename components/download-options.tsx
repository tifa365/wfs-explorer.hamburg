...
export function DownloadOptions({
  ...
  const showDownloadAllOption =
    totalFeatureCount !== null && totalFeatureCount > 0 && loadedFeatureCount < totalFeatureCount
  ...
  const handleDownload = async () => {
    try {
      ...
      const data = await fetchWfsDataForDownload(wfsUrl, layer.id, effectiveMaxFeatures, layer, useNativeProjection)
      ...
      const projectionLabel = useNativeProjection ? layer.defaultProjection || "Native" : "WGS84"
      const filename = `${layer.id.replace(/:/g, "_")}_${projectionLabel}.geojson`
      ...
      <CardContent>
        <div className="space-y-4">
          <div className="space-y-2">
            <h3 className="text-base font-medium mb-2">{t("projectionFormat")}</h3>
            {!projectionIssue ? (
              <Tabs
                defaultValue="wgs84"
                value={downloadType}
              ...
              >
                <TabsList className="grid w-full grid-cols-2">
                  <TabsTrigger value="wgs84" className="flex items-center gap-1">
                    <span className="font-medium">WGS84</span>
                    <span className="text-xs text-muted-foreground">(EPSG:4326)</span>
                  </TabsTrigger>
                  <TabsTrigger value="native" className="flex items-center gap-1">
                    <span className="font-medium">{t("nativeProjection")}</span>
                    <span className="text-xs text-muted-foreground">({layer.defaultProjection || t("unknown")})</span>
                  </TabsTrigger>
                </TabsList>
              </Tabs>
            ) : (
              <div className="text-sm bg-muted p-2 rounded">
                {t("nativeProjectionOnly")} ({layer.defaultProjection || t("unknown")})
              </div>
            )}
          </div>
              ...
              <div className="grid gap-1.5 leading-none">
                ...
                {totalFeatureCount && (
                  <p className="text-xs text-muted-foreground">
                    {t("totalAvailable")}: {totalFeatureCount.toLocaleString()} {t("features")}
                  </p>
                )}
              </div>
            </div>
