...
export function AttributeExplorer({
  ...
  const [attributeValueSuggestions, setAttributeValueSuggestions] = useState<string[]>([])
  ...
  const getSuggestionsForAttribute = (attribute: string): string[] => {
    ...
    data.features.forEach((feature: any) => {
      if (feature.properties && feature.properties[attribute] !== undefined && feature.properties[attribute] !== null) {
    ...
    return [...filteredFeatures].sort((a, b) => {
      ...
      if (sortConfig.key === "id") {
        ...
        if (!isNaN(aIdNum) && !isNaN(bIdNum)) {
          return sortConfig.direction === "asc" ? aIdNum - bIdNum : bIdNum - aIdNum
        }
  ...
  const paginatedFeatures = sortedFeatures.slice(startIndex, startIndex + pageSize)
    ...
    <div className="space-y-4">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="space-y-2">
          ...
          <Select value={selectedAttribute || ""} onValueChange={onSelectAttribute}>
        ...
        <div className="space-y-2">
          ...
          <div className="relative">
            <Input
              id="filter-input"
              placeholder={
                selectedAttribute ? `${t("filterByAttribute")} ${selectedAttribute}` : t("selectAttributeFirst")
              }
              value={filter}
              onChange={(e) => {
      ...
      <div className="border rounded-md overflow-auto" ref={tableRef} style={{ maxHeight: "500px" }}>
        <Table>
          <TableHeader>
            <TableRow>
              {onViewFeatureOnMap && <TableHead className="w-[60px] text-center">Map</TableHead>}
              <TableHead
              ...
              {attributes.map((attr) => (
                <TableHead
                  key={attr}
                  className={`cursor-pointer hover:bg-muted/50 text-xs font-medium uppercase text-gray-500 ${selectedAttribute === attr ? "bg-muted" : ""}`}
          ...
          <TableBody>
                ...
                {onViewFeatureOnMap && (
                  <TableCell className="text-center">
                    <Button
                      ...
                      title={feature.geometry ? t("viewOnMap") : t("noGeometry")}
                    >
                      <MapPin className="h-4 w-4" />
                    </Button>
                  </TableCell>
                )}
                <TableCell className="font-medium">{feature.id || startIndex + index + 1}</TableCell>
                {attributes.map((attr) => (
                  <TableCell key={attr} className={selectedAttribute === attr ? "bg-muted/30" : ""}>
                    {feature.properties[attr] !== undefined ? String(feature.properties[attr]) : "N/A"}
                  </TableCell>
                ))}
              </TableRow>
            ))}
            ...
            {paginatedFeatures.length === 0 && (
              <TableRow>
                <TableCell colSpan={attributes.length + (onViewFeatureOnMap ? 2 : 1)} className="text-center h-24">
                  {t("noResults")}
                </TableCell>
              </TableRow>
            )}
      ...
      <div className="text-xs text-gray-500">
        {t("showing")} {paginatedFeatures.length} {t("of")} {sortedFeatures.length} {t("entries")}
        {sortedFeatures.length !== features.length && ` (${t("filteredFromTotal")} ${features.length} ${t("total")})`}
      </div>
    </div>
  )
}
